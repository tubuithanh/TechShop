...
(async () => {
  ...
  try {
    // Kết nối DB để đọc cấu hình admin đã lưu (Cấu hình hệ thống > Email); không có thì dùng biến môi trường
    ...
    console.log(mode === 'demo' ? 'Chưa gửi thật - cấu hình trong Admin > Cấu hình hệ thống > Email, hoặc điền SMTP_*/RESEND_API_KEY trong .env' : `Đã gửi email thử tới ${to}, hãy kiểm tra hộp thư (cả mục Spam).`);
